...
*
**/
void Get_AccGyro_ICM42688_IIC(void)
{
    uint8_t data[12];
	IIC_ReadBytes(&i2c1, DeviceAddress_ICM42688, ACCEL_DATA_X1, 12, data);
	for(int i = 0; i < 3; i++)
    IMU.acc[i] = icm42688_iic_acc_inv * (int16_t)(((int16_t)data[2*i] << 8) | data[2*i+1]);
	for(int i = 0; i < 3; i++)
    IMU.gyro[i] = icm42688_iic_gyro_inv * (int16_t)(((int16_t)data[2*i + 6] << 8) | data[2*i+7]);
}
