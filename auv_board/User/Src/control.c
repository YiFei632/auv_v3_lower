...
void telecontrol_mode_servo()
{
	if(telecontrol.is_receive)
	{
		...
		__HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_3, (uint16_t)telecontrol_servo_value[0]);
		__HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_4, (uint16_t)telecontrol_servo_value[3]);
		//__HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_3, (uint16_t)telecontrol_servo_value[2]);
		//__HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_4, (uint16_t)telecontrol_servo_value[3]);
...
float hz, A, derta;
float angle;
float telecontrol_servo_value_sin[4];
void telecontrol_mode_servo_sin()
{
	if(telecontrol.is_receive)
	{
		...
		for(int i = 0; i < 4; i++)
		{
			telecontrol_servo_value_sin[i] = SERVO_MEDIAN + A * sin((timestamp / 100.) * hz * 6.28) + derta;
			telecontrol_servo_value_sin[i] = angle + SERVO_MEDIAN;
		}
		
#ifdef FLYING_FISH
		__HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_1, (uint16_t)telecontrol_servo_value_sin[0]);
		__HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_2, (uint16_t)telecontrol_servo_value_sin[1]);
#endif
		//__HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_3, (uint16_t)telecontrol_servo_value[2]);
...
void telecontrol_mode_motor()
{
	if(telecontrol.is_receive)
	{
		...
		for(int i = 0; i < 4; i++)
		{
			...
			if (rocker_temp < 0.57)
				set_value[i] = SERVO_LOW;
			else
				set_value[i] = 2 * (rocker_temp - 0.5) * (SERVO_HIGH - SERVO_LOW) + SERVO_LOW;
		...
		__HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_1, (uint16_t)telecontrol_motor_value[0]);
		__HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_2, (uint16_t)telecontrol_motor_value[1]);
		__HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_3, (uint16_t)telecontrol_motor_value[2]);
		__HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_4, (uint16_t)telecontrol_motor_value[3]);
	}
}
...
void telecontrol_mode_pump()
{
	...
	__HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_1, temp_pump[4]);//pump
	
	__HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_2, temp_pump[0]);//chage stiff 
	__HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_3, temp_pump[1]);
